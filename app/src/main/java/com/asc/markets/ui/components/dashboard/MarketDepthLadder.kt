...
fun MarketDepthLadder(symbol: String, price: Double) {
    val storePair = BinanceDataStore.pairSnapshot(symbol) ?: MarketDataStore.pairSnapshot(symbol)
    val safePrice = price.takeIf { it.isFinite() && it > 0.0 } ?: storePair?.price?.takeIf { it.isFinite() && it > 0.0 } ?: 1.0
    val pairFlow = if (BinanceDataStore.isUsdtSymbol(symbol)) {
        BinanceDataStore.pairFlow(symbol)
    } else {
        MarketDataStore.pairFlow(symbol)
    }
