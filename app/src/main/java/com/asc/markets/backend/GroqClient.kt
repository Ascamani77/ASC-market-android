 ...
 *
 * NOTE: this client reads the API key from `BuildConfig.GROQ_API_KEY` which is populated
 * from your Gradle/project properties (see `app/build.gradle.kts`). Do NOT commit API keys
 * into source control. Put the key in `local.properties` as `GROQ_API_KEY=...` or supply
...
object GroqClient {
    ...
    suspend fun chatCompletion(prompt: String, model: String = "llama-3.3-70b-versatile"): String = withContext(Dispatchers.IO) {
        // Only use the build-time injected key. Do NOT accept runtime/pasted API keys.
        val key = BuildConfig.GROQ_API_KEY
        require(key.isNotBlank()) { "GROQ_API_KEY not set in BuildConfig. Add it to app/local.properties as GROQ_API_KEY=... and rebuild the app." }
        ...
        if (code !in 200..299) throw IllegalStateException("Groq API error ($code): $respText")
