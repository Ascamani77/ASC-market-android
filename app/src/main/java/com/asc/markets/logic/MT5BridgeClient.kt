...
class MT5BridgeClient(
    ...
    suspend fun getTick(symbol: String): TickData? = withContext(Dispatchers.IO) {
        try {
            val request = Request.Builder()
                .url("http://${bridgeUrl}/tick?symbol=${symbol}")
                .get()
                .build()
    ...
    suspend fun getHistoricalBars(
        ...
        try {
            val request = Request.Builder()
                .url("http://${bridgeUrl}/historical?symbol=${symbol}&timeframe=${timeframe}&limit=${limit}")
                .get()
                .build()
        ...
        try {
            ...
            ws = wsClient!!.newWebSocket(request, object : WebSocketListener() {
                override fun onOpen(webSocket: WebSocket, response: Response) {
                    priceUpdates.keys.forEach { sendWebSocketMessage("subscribe", it) }
                }
                ...
                override fun onMessage(webSocket: WebSocket, text: String) {
                    try {
                        ...
                        val bid = json.getDouble("bid")
                        val ask = json.getDouble("ask")
                        val time = json.getLong("time")
                        val volume = json.getInt("volume")
                        priceUpdates[symbol]?.invoke(bid, ask, time, volume)
                    } catch (e: Exception) { e.printStackTrace() }
                }

                override fun onFailure(webSocket: WebSocket, t: Throwable, response: Response?) {
                    scope.launch { delay(5000); connectWebSocket() }
                }
            })
        } catch (e: Exception) { e.printStackTrace() }
    }
