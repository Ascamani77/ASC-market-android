...
class TiingoFxRestClient(
    ...
    suspend fun fetchTopPairs(symbols: List<String>): List<ForexPair> = withContext(Dispatchers.IO) {
        ...
        if (now < rateLimitedUntilMillis) {
            Log.w(TAG, "Skipping Tiingo FX REST top; rate limited for ${(rateLimitedUntilMillis - now) / 1000}s")
            return@withContext emptyList()
        }
        if (now < nextAllowedRequestMillis) {
            Log.i(TAG, "Skipping Tiingo FX REST top; cooldown for ${(nextAllowedRequestMillis - now) / 1000}s")
            return@withContext emptyList()
        }
        ...
        try {
            client.newCall(request).execute().use { response ->
                ...
                if (!response.isSuccessful) {
                    if (response.code == 429) {
                        ...
                        Log.w(TAG, "Tiingo FX REST rate limited; backing off for ${retryAfterMillis / 1000}s")
                    }
                    Log.w(TAG, "Tiingo FX REST top failed code=${response.code} body=${body.take(240)}")
                    return@withContext emptyList()
                }
                ...
                Log.i(TAG, "Tiingo FX REST top returned ${pairs.size}/${tickers.size} prices")
            ...
            Log.w(TAG, "Tiingo FX REST top request failed: ${e.message}", e)
            emptyList()
        }
    }
    ...
    private fun parseTopItem(item: org.json.JSONObject): ForexPair? {
        ...
        val bid = item.optDouble("bidPrice", Double.NaN)
        val ask = item.optDouble("askPrice", Double.NaN)
        val mid = item.optDouble("midPrice", Double.NaN)
    ...
    private companion object {
        private const val TAG = "TiingoFxREST"
        private const val PREFS_NAME = "asc_prefs"
        private const val KEY_RATE_LIMITED_UNTIL = "tiingo_fx_rest_rate_limited_until"
        private const val KEY_NEXT_ALLOWED_REQUEST_AT = "tiingo_fx_rest_next_allowed_request_at"
        private const val TOP_URL = "https://api.tiingo.com/tiingo/fx/top"
