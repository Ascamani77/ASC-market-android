...
/**
 * Pepperstone Demo Chart Service
 * Connects to demo cTrader bridge (port 8083) for demo account data
 */
class PepperstoneDemoChartService(
    ...
    private fun setupRedis() {
        try {
            ...
            Log.i(TAG, "Redis Pool initialized at $redisHost:$redisPort for Pepperstone Demo ticks")
        } catch (e: Exception) {
        ...
        scope.launch {
            try {
                jedisPool?.resource?.use { jedis ->
                    ...
                    Log.d(TAG, "Published Pepperstone Demo tick to Redis: ${quote.name} @ ${quote.lastPrice}")
                }
            } catch (e: Exception) {
    ...
    fun connect() {
        ...
        webSocket = client.newWebSocket(request, object : WebSocketListener() {
            override fun onOpen(webSocket: WebSocket, response: Response) {
                Log.i(TAG, "Pepperstone Demo WebSocket connected")
            ...
            override fun onFailure(webSocket: WebSocket, t: Throwable, response: Response?) {
                Log.e(TAG, "Pepperstone Demo chart WebSocket failure: ${t.message}")
                SystemTelemetry.recordConnectionEvent("CTRADER_DEMO", "CONNECTION_FAILED")
                scheduleReconnect()
            }

            override fun onClosed(webSocket: WebSocket, code: Int, reason: String) {
                Log.i(TAG, "Pepperstone Demo WebSocket closed: $reason")
                SystemTelemetry.recordConnectionEvent("CTRADER_DEMO", "CONNECTION_CLOSED ($reason)")
    ...
    private fun handleTick(payload: JSONObject) {
        ...
        SystemTelemetry.recordTick("CTRADER_DEMO", 5.0)  // Changed from PEPPERSTONE_DEMO_CHART to match telemetry tracking
    ...
    private companion object {
        private const val TAG = "PepperstoneDemoChart"
    }
}
